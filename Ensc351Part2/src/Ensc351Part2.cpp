...
void termFunc(int termNum)
{
	...
	if (termNum == Term1) {
		...
		// we need to double check which threads to close
		PE(myClose(daSktPrT1M[MediumSkt]));
//		PE(myClose(daSktPrT1M[termNum]));
		...
		// char *senderFileName = "/etc/printers/epijs.cfg"; // for QNX 6.5 target
		...
		// we need to double check which threads to close
		PE(myClose(daSktPrMT2[MediumSkt]));
